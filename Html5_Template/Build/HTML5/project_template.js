...
function showLoadingScreen() {
  ...
  loadingDiv.id = 'loading'; // Give it an ID for styling
  loadingDiv.style.position = 'absolute';
  loadingDiv.style.top = '50%';
  loadingDiv.style.left = '50%';
  loadingDiv.style.transform = 'translate(-50%, -50%)';
  loadingDiv.style.zIndex = '1'; // Ensure it appears on top of the game

  // Create the loading GIF image
  var gifImage = document.createElement('img');
  gifImage.src = LoadingThrobber; // Link to your GIF
  gifImage.alt = 'Loading...';

  // Append the image to the loading div
  loadingDiv.appendChild(gifImage);

  // Append the loading div to the body
  document.body.appendChild(loadingDiv);
}

// Remove the loading GIF when the game is ready
function hideLoadingScreen() {
  var loadingDiv = document.getElementById('loading');
  if (loadingDiv) {
    loadingDiv.style.display = 'none'; // Hide the loading GIF
  }
}
...
var canvasWindowedScaleMode = %CANVASSCALEMODE%;  //►►►►►►►►►►►►►►►►►►►►►►►► Set this one to 1 for itch - Project settings Stretch
